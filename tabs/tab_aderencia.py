...
def _gantt_aderencia(df_agente_dia: pd.DataFrame, df_escala_agente_dia: pd.DataFrame, agente_gantt: str, data_gantt: datetime.date):
    # df_agente_dia já deve vir filtrado para o agente e dia específicos
    ...
    for h in range(24):
        fig.add_vline(
            x=datetime.combine(data_gantt, time(h, 0, 0)), # Usar datetime object diretamente
            line_width=0.5,
            line_dash="dot",
            line_color="gray",
...
def _calcular_metricas_aderencia(df_filtrado: pd.DataFrame, df_escala: pd.DataFrame):
    if df_filtrado.empty or df_escala.empty:
        return pd.DataFrame()

    # Garantir que 'data' em df_filtrado seja datetime.date para merge
    ...
    df_pivot = df_pivot.rename(columns={"data_only": "data"}) # Renomear de volta para 'data' para o merge
    ...
    df_escala_copy['data'] = df_escala_copy['data'].dt.date # Converter para date object
...
def render(df_hist_filtrado_global: pd.DataFrame, df_escala: pd.DataFrame):
    ...
    df_metricas_aderencia = _calcular_metricas_aderencia(df_hist_filtrado_global, df_escala)
    ...
    agentes_disponiveis_gantt = df_hist_filtrado_global["agente"].unique().tolist()
    ...
    with col_gantt2:
        datas_disponiveis_agente = sorted(df_hist_filtrado_global[df_hist_filtrado_global["agente"] == agente_gantt]["data"].unique().tolist())
    ...
    if agente_gantt and data_gantt:
        # Filtrar df_hist_filtrado_global para o agente e data específicos para o Gantt
        df_gantt_data = df_hist_filtrado_global[
            (df_hist_filtrado_global["agente"] == agente_gantt) &
            (df_hist_filtrado_global["data"] == data_gantt)
        ]
